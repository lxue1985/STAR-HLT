...
/*    for(int i = 1; i < 21; i++){
    ((TH1F *)getPlotByIndex(1)->getHisto(0)->histo)->SetBinContent(i,i);
    }
*/
    for(int i= 1; i< 10;i++){
    ((TH1F *)getPlotByIndex(1)->getHisto(1)->histo)->SetBinContent(i,20-i);
    }
    getPlotByIndex(1)->getHisto(1)->histo->SetLineColor(2);
