...
StHltMaker::StHltMaker(const char *name) : StRTSBaseMaker("hlt",name)
    , mStEvent(0), mStHltEvent(0)
	, mnHighPt(0), mnHeavyFragment(0),mnDielectron(0)
{
...
StHltEvent *StHltMaker::GetHltEvent()
{
	...
	if (mStEvent) {
		...
		if ( !HltEvent )  {
			...
			HltEvent = new StHltEvent();    /// Need to create the StHltEvent
...
void StHltMaker::ProcessBank(const HLT_EVE  *hlt_eve)
{
	...
	mStHltEvent->setVersion((unsigned int) hlt_eve->version) ;  /// version is a date from which on dedx gain was saved 
	mStHltEvent->setTriggerReasonBitOred((unsigned int) hlt_eve->hltDecision) ; /// StHltTriggerReason::reasonBit with "OR" operated.
	mStHltEvent->setT0(hlt_eve->T0) ;  /// vpd start time
	mStHltEvent->setVpdVertexZ(hlt_eve->vpdVertexZ);

	if( hlt_eve->version >= HLT_GL3_VERSION){  /// HLT_GL3_VERSION is the start date when dedx gain saved
...
void StHltMaker::ProcessBank(const HLT_GT   *hlt_gt)
{
	...
	for(u_int i=0 ; i < hlt_gt->nGlobalTracks; i++){
		...
		aTrack->setReserved(hlt_gt->globalTrack[i].reserved);
...
void StHltMaker::ProcessBank(const HLT_PT   *hlt_pt)
{
	...
	for(u_int i=0;i<hlt_pt->nPrimaryTracks;i++){
		...
		aTrack->setReserved(hlt_pt->primaryTrack[i].reserved);
...
void StHltMaker::ProcessBank(const HLT_HF   *hlt_hf)
{
    ...
    mnHeavyFragment = hlt_hf->nHeavyFragments;
...
void StHltMaker::FillHeavyFragment(StHltEvent* event)
{
	...
	for(u_int i=0 ; i<mnHeavyFragment ; i++){
...
void StHltMaker::FillDielectron(StHltEvent* event)
{
	...
	for(u_int i=0 ; i<mnDielectron ; i++){
		...
		dielectron->setDaughter1GlobalTrack(daughter2globalTrack);
		dielectron->setDaughter1PrimaryTrack(daughter2primaryTrack);
		dielectron->setDaughter1BTofHit(daughter2bTofHit);
		dielectron->setDaughter1BEmcTowerHit(daughter2bEmcHit);
		dielectron->setDaughter1GlobalTrackSN(daughter2node->globalTrackSN());
		dielectron->setDaughter1PrimaryTrackSN(daughter2node->primaryTrackSN());
		dielectron->setDaughter1TofHitSN(daughter2node->tofHitSN());
		dielectron->setDaughter1EmcTowerSN(daughter2node->emcTowerSN());
		dielectron->setDaughter1BEmcMatchPhiDiff(daughter2node->bEmcMatchPhiDiff());
		dielectron->setDaughter1BEmcMatchZEdge(daughter2node->bEmcMatchZEdge());
		dielectron->setDaughter1BTofProjChannel(daughter2node->bTofProjChannel());
		dielectron->setDaughter1BTofCellLocalY(daughter2node->bTofCellLocalY());
		dielectron->setDaughter1BTofCellLocalZ(daughter2node->bTofCellLocalZ());
		dielectron->setDaughter1BTofPathLength(daughter2node->bTofPathLength());
		dielectron->setDaughter1Beta(daughter2node->beta());
		dielectron->setDaughter1Tof(daughter2node->tof());
...
void StHltMaker::FillTriggerReason(StHltEvent* event)
{
	...
	for(u_int i=0 ; i<VecDiElectron.size() ; i++){
		...
		aTriggerReason->setReasonBit(0x20000);
	...
	for(u_int i=0 ; i<VecHighPt.size() ; i++){
		...
		StHltTriggerReason *aTriggerReason = new  StHltTriggerReason();
		aTriggerReason->setReasonBit(0x10000);
	...
	for(u_int i=0 ; i<VecHeavyFragment.size() ; i++){
		...
		StHltTriggerReason *aTriggerReason = new  StHltTriggerReason();
		aTriggerReason->setReasonBit(0x40000);
