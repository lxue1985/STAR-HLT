...
//-----------------------------------------------------------------
   ...
   // QIU Hao modified this 20090202
   ...
   double stot = getPara()->bFieldPolarity*float(q)*dangle*rc;
   zp   = z0 - stot * tanl ;
...
**:
**:>---------------------------------------------------------------------------------------*/
double FtfBaseTrack::pathLength(double Rx, double Ry, double Rz, double Nx, double Ny, double Nz ,double *HitPosition)
{
 // THIS FUNCTION CAN ONLY BE USED WITH MAGNETIC FIELD > 0 !!!
 ...
 // curvature == kapa
 double Curvature = fabs(0.3*q*Bfield)/pt; // kapa
 ...
 if(HitPosition){
	HitPosition[0] = x0+(cos(a)-cos(Phase))/Curvature;
	HitPosition[1] = y0+(sin(a)-sin(Phase))/Curvature;
	HitPosition[2] = z0+s*SinDipAngle;
 }
