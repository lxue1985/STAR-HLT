...

{
   ...
   online_tracking_TpcHitMap* tpcHitMap = new online_tracking_TpcHitMap();
    ...
    bemcCalib->loadTextMap("emcmap.txt");
...
//for(int ei=0;ei<mNEvents;ei++){
//chain->GetEntry(ei);
//cout<<ei<<"    "<<TrackEvent->mNMcHit<<endl;
    ...
    for(;;){
        ...
        int what=GL3_READ_TPC_TRACKS;
        daq_dta *dd;
        online_tracking_collector eventbuilder(NULL,bemcCalib,eemcCalib);
        eventbuilder.resetEvent();
        bField=getfield(rdr);
        eventbuilder.setBField(bField);
        online_tracking_sector *tracker = new online_tracking_sector(bField,"/RTS/conf/L3/map.bin",0);
		tracker->setTpcHitMap(tpcHitMap);


		eventbuilder.emc.readFromDaqReader(rdr);
...
/*            tracker->nHits=0;
            ...
            tracker->sector_ID=i;
            tracker->Tracking_load_cluster(Tracking_pTPC);
            //tracker->Tracking_load_cluster(TrackEvent,i);
			//cout<<"load_time  "<<tracker->diftime<<endl;
            float seed[5];
            seed[0]=43;
            seed[1]=-1;
            seed[2]=1;
            seed[3]=0;
            seed[4]=1;
            //tracker->Tracking_track_sector(seed,1,(char *)sectp,size);

			
			if((i%2) == 1) {

            tracker->Tracking_track_sector((char *)sectp,size);
			//tracker->Tracking_track_sector(seed,1,0,(char *)sectp,size);

            LOG(DBG, "SECP size = %d",sectp->bh.length*4 + sectp->banks[0].len*4);


            //	int n = gl3->readSectorTracks((char *)sectp);
            int n = eventbuilder.Tracking_readSectorTracks((char *)sectp);
			cout<<n<<" tracks in "<<i<<" sector "<<endl;
			

			//cout<<"track_time  "<<((float)tracker->diftime)/((float)n)<<endl;
			}
