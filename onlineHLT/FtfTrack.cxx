...
PROFILER_DECLARE ;
void      FtfTrack::transhit( FtfHit   *thisHit){
	thisHit->txp=thisHit->xp*cos+thisHit->yp*sin;
	thisHit->typ=thisHit->yp*cos-thisHit->xp*sin;
}

void      FtfTrack::transpara(double b ){
   ...
   ts12Xy=s12Xy*cos+g1Xy*sin;
   tg1Xy=g1Xy*cos-s12Xy*sin;
   tg2Xy=g2Xy*(cc-ss)+(g22Xy-s22Xy)*cs;

   ts22Xy=0.5*s22Xy*(1+cc-ss)+0.5*g22Xy*(1-cc+ss)+2*g2Xy*cs;

   tg22Xy=0.5*s22Xy*(1-cc+ss)+0.5*g22Xy*(1+cc-ss)-2*g2Xy*cs;
...
//
  ...
  s12Xy = s12Xy + thisHit->wxy * thisHit->txp ;
  s22Xy = s22Xy + thisHit->wxy * square(thisHit->txp) ;
  g22Xy = g22Xy + thisHit->wxy * square(thisHit->typ) ;
  g1Xy  = g1Xy  + thisHit->wxy * thisHit->typ ;
  g2Xy  = g2Xy  + thisHit->wxy * thisHit->txp * thisHit->typ ;  
  ...
  g22Xy = g22Xy + thisHit->wxy * square(thisHit->yp) ;
...
//
   ...
   if(this->nHits==4&&this->transed==false){
	   this->transpara(this->a2Xy);
	   
   }

   if(this->transed){
	   this->transhit(candidateHit);
   }
...
//
   if(this->transed){
   temp = (a2Xy * candidateHit->txp - candidateHit->typ + a1Xy) ;
