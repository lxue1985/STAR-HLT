/*:>-------------------------------------------------------------------
**: File:     online_tracking_sector.cxx 
**: HISTORY:
...
**:                         (online_tracking_sector::fillUSTracks) 
...
//******************************************************************
int online_tracking_sector::Tracking_track_sector ( char *out_buf,int &out_buf_used )
{
	timer_run();

	processSector();

	// Tonko: I pass in the max bytes
	int max_bytes = out_buf_used ;

	//out_buf_used=fillTracks(szSECP_max, (char *)out_buf, 0);
	out_buf_used=fillTracks(max_bytes, (char *)out_buf, 0);

	timer_stop();
	//timer_print(" online_tracking_sector::Tracking_track_sector full ");

	LOG(NOTE,"Used %d/%d bytes",out_buf_used,max_bytes) ;

	return nTracks ;
    
}

int online_tracking_sector::Tracking_track_sector (float* seed,int seednum , char *out_buf,int &out_buf_used )
{
 timer_run();
    processSector(seed,seednum);
    out_buf_used=fillTracks(szSECP_max, (char *)out_buf, 0);

	timer_stop();
	//timer_print(" online_tracking_sector::Tracking_track_sector seed ");
...
int online_tracking_sector::Tracking_track_sector (float* detadphi,int region_num, int region_type, char *out_buf,int &out_buf_used)
{
 timer_run();

 if(region_type==0){ //for eta and phi cut
    processSector(detadphi,region_num,region_type,0);
...
int online_tracking_sector::Tracking_load_cluster ( tpc_t *in_pTPC )
{
 timer_run();
    this->Tracking_pTPC=in_pTPC;
    readSectorFromEvpReader(this->sector_ID+1);
	timer_stop();
	//timer_print(" online_tracking_sector::Tracking_load_cluster ");
    return nHits;
    
}
int online_tracking_sector::Tracking_load_cluster ( mTrackEvent *in_mTrackEvent,int secID)
{
...
int online_tracking_sector::Tracking_load_cluster (int MCtrack_number,float* vertex, mTrackEvent *in_mTrackEvent,int secID,int *ID_list,int* hit_number_list)
{
...
int online_tracking_sector::canItBeMerged ( FtfTrack* tTrack ) 
...
//   fill Hits
//******************************************************************
int online_tracking_sector::fillHits ( unsigned int maxBytes, char* buff, unsigned int token ) 

{
    ...
    
    {
        ...
        LOG (ERR, "online_tracking_sector::fillHits: buffer with %d bytes too small \n", maxBytes ) ;
        return 0 ;
    ...
    
    {
        ...
        
        {
            ...
            
            {
                ...
                LOG (ERR, "online_tracking_sector:fillHits: we got a problem track %d row hit %d \n",
    ...
    
    {
        ...
        LOG (ERR, "online_tracking_sector:fillHits: Warning! Actual number of hits written %d total nHits %d \n", 
        counter, nHits );
...
//   fill tracks
//******************************************************************
int online_tracking_sector::fillTracks ( int maxBytes, char* buff, unsigned int token ) 

{
    ...
    //    + counter * sizeof(struct local_track) ; 
    char *endOfData = pointer + headSize
      + counter * sizeof(struct local_track)*2 ;   // Qiu Hao added primary tracks
    // end
    ...
    
    {
        ...
        LOG(ERR, "online_tracking_sector::fillTracks: %d bytes needed, %d max, too short a buffer \n ",  
        nBytes, maxBytes ) ;
        return -1 ;
    ...
    head->bh.format_ver = DAQ_RAW_FORMAT_VERSION ;
    head->bh.byte_order = DAQ_RAW_FORMAT_ORDER ;
    head->bh.format_number = 1; 
    // 1 means only pointing to local_tracks
    head->bh.token      = token;
    head->bh.w9         = DAQ_RAW_FORMAT_WORD9;
    head->bh.crc        = 0; 
    //don't know yet....
    ...
    head->cpuTime  = (unsigned int)rint(cpuTime * 1000000);   
    // to get microsec
    head->realTime = (unsigned int)rint(realTime * 1000000) ; 
    // same here
    head->xVert  = (int)rint(para.xVertex * 1000000);   
    // to get cm*10**-6
    ...
    //Qiu Hao add to get primary tracks
        //
	track[i].getPara()->vertexConstrainedFit = 1;
	track[i].fitHelix();
        uSTrack->id    = track[i].id ;
        if ( canItBeMerged ( &(track[i]) ) ) 
...
//   get nr of tracks
//******************************************************************
int online_tracking_sector::getNrTracks (void) 
...
//******************************************************************
int online_tracking_sector::setup ( int maxHitsIn, int maxTracksIn ) 
...
//******************************************************************
//#define TRDEBUG
//int online_tracking_sector::readMezzanine (int sector,        int readOutBoard,
...
//    // LOG(ERR, "online_tracking_sector::readMezzaninne:  Found %d rows.\n",rows) ;
//    for (i=0; i<rows; i++) 
//    
//    {
//
...
//            //l3Log("online_tracking_sector: Read cluster %i\n", j);
//            struct xt 
//            
//            {
//
...
//            
//            #ifdef MDEBUG
//            LOG(ERR, "online_tracking_sector:readMezzaninne row %d \n", row ) ;
...
//            {
//
//                LOG(ERR, "Error - online_tracking_sector:read: Hit array too small: %d + %d > %d \n",
...
//     ppy:    modified to look at byte order
//******************************************************************
//int online_tracking_sector::readSector ( struct bankHeader *bank ) 
//
//{
//
...
//    {
//
//        LOG(ERR, "Error - online_tracking_sector::readSector: Wrong sector %d!\n",sectorNr);
//        return -1 ;
//        
//    }
//
...
//                para.phiMin = 0. ;
//                para.phiMax = 1.1 ; 
//                // 60 degress a bit more than a rad
//                para.phiShift = 0.8 ; 
//                // ~45 degrees
//                
//            }
...
//
int online_tracking_sector::setTrackingAngles(int sector)

{
    ...
    //    if(para.phiMin > para.phiMax) para.phiMin -= 2*M_PI;
    if (sector == 3 || sector == 21)
    {          
        para.phiMin = 0.;
        para.phiMax = M_PI/6.; 
        para.phiShift = M_PI/12.; 
        
    }
    //    cout<<"para.phiMin : "<<para.phiMin<<"    para.phiMax : "<<para.phiMax<<endl;
...
int online_tracking_sector::readSectorFromESB(int sector, char *mem, int len) 
{
	...
	while(p_buff < end_buff) {
		...
		for(u_int i=0;i<cou;i++) {
			...
			hitP->row = row;
			hitP->sector = sector;		// Tonko: do we really need this here for sector tracking???
			hitP->x = xyz[0];
			hitP->y = xyz[1];
			hitP->z = xyz[2];
			hitP->q = c.charge;			
...
//
int online_tracking_sector::readSectorFromEvpReader(int sector) 

{
    ...
    
    {
        ...
        #else /* OLD_DAQ_READER */
        for(int j=0;j<Tracking_pTPC->cl_counts[r];j++) 
        
        {

            tpc_cl *c = &Tracking_pTPC->cl[r][j];
      ...
      double xyz[3];
      getTpcHitMap()->mapping(xyz, r+1, c->p, c->t);
      ...
      hitP->z = xyz[2];
            ...
            hitP->flags = c->flags;
            hitP->q = c->charge;
			//cout<<"c->flags  "<<c->flags<<" c->charge "<<c->charge<<endl;
            hitP->hardwareId = 0;
			hitP->Mc_key=0;
			hitP->Mc_track_key=0;
...
//
//int online_tracking_sector::readSector (DATAP *datap, int sector) 
//
//{
//
//    if ( sector < 1 && sector > 24 ) 
...
////
////******************************************************************
//int online_tracking_sector::readSector ( struct TPCSECLP *seclp1, struct TPCSECLP *seclp2 ) 
//
//{
//
//    struct TPCRBCLP *rbclp ;
...
//                    {
//
//                        LOG(ERR, "online_tracking_sector::readSector:       MZCLD %d exists!\n", kMz+1) ;
//                        
//                    }
//
//                    
...
//                    {
//
//                        LOG(ERR, "online_tracking_sector:readSector: wrong reading mezzanine \n" ) ;
//                        return -1;
//                        
//                    }
//
...
//    Process
//*******************************************************************
int online_tracking_sector::processSector ( )
...
{ 
    ...
    //
    if ( debugLevel > 0 ) 
    LOG(ERR," online_tracking_sector::process: tracks %i Time: real %f cpu %f\n", 
...
int online_tracking_sector::processSector (float* seed,int seednum )
...
int online_tracking_sector::processSector (float* seed,int seednum,int seedlayer,float seed_eta_phi_cut  )
...
{ 
    ...
    para.eventReset = 1 ;
    nTracks         = 0 ;
    process (seed,seednum,seedlayer,seed_eta_phi_cut) ;
    if (para.dEdx) dEdx();
    //
    if ( debugLevel > 0 ) 
    LOG(ERR," online_tracking_sector::process: tracks %i Time: real %f cpu %f\n", 
...
#if 0
int online_tracking_sector::processData(TPCSECLP* seclp, char* trackBuffer, 
...
//********************************************************************
int online_tracking_sector::dEdx ( ) 
...
//***************************************************************
int online_tracking_sector::setParameters ( ) 

{
    ...
    para.xVertex          = 0.35F    ;
    para.yVertex          = -0.1F    ;
    para.rVertex          = sqrt(pow(para.xVertex, 2)+pow(para.yVertex, 2))   ;
    para.zVertex          = 0.F    ;
    para.dxVertex         = 0.05F ;
    para.dyVertex         = 0.05F ;
    para.phiVertex        = atan2(para.yVertex, para.xVertex)    ;
    para.zMax             = 205. ;
    return 0;
    
}

void online_tracking_sector::readHLTparameters(char* HLTparameters)
{
  tpcHitMap->readHLTparameters(HLTparameters);
  ...
  if(!ifs.fail())
    while(!ifs.eof())
      {
	ifs>>parameterName;
	if(parameterName == "xVertex") ifs>>para.xVertex;
	if(parameterName == "yVertex") ifs>>para.yVertex;
	if(parameterName == "minHitsPerTrack") ifs>>para.minHitsPerTrack;
      }
  para.rVertex   = sqrt(pow(para.xVertex, 2)+pow(para.yVertex, 2))   ;
  para.phiVertex = atan2(para.yVertex, para.xVertex)    ;
...
void online_tracking_sector::readBeamline(char* beamlineFile)
{
	int run_num[50]; double beamX[50] , beamY[50];
	int end_num = 0; int nlines = 0;
	string str;

	// Tonko: this is all done badly... there are numerous
	...
	if(pro_num > 0) {
		para.xVertex = beamXSum/pro_num ;
		para.yVertex = beamYSum/pro_num ;
	}
	else {
		// leave default...
	}
...
void online_tracking_sector::put_bField(double bField)
{
...
int online_tracking_sector::readScalers(char* scalerFile)
{
  LOG(WARN,"Don;t use this function!") ;
  ...
  tpcHitMap->setSpaceChargeFromScalerCounts((int *)scalerCounts);
