...
gl3Track* gl3Track::merge ( FtfContainer *trackArea ){
   ...
   if ( i_phi < 0 ) {
//    l3Log ( " Track phi index too low  %d \n", i_phi ) ;
      i_phi = 1 ;
   }
   if ( i_phi >= getPara()->nPhiTrackPlusOne ) {
//    l3Log ( " Track phi index too high %d \n", i_phi ) ;
   ...
   if ( i_eta <= 0 ) {
//    l3Log ( " Track eta index too low  %d \n", i_eta ) ;
      i_eta = 1 ;
   }
   if ( i_eta >= getPara()->nEtaTrackPlusOne ) {
//    l3Log ( " Track eta index too high %d \n", i_eta ) ;
...
//
	    ...
	    //
//             l3Log ( "gl3Track: close tracks merged !!! \n " ) ;
//             i_track->Print(30);
//             Print(30);
	    ...
	    //   double dz0 = fabs(z0-i_track->z0);
	       ...
	       //
//                   l3Log ( "gl3Track: far apart tracks merged !!! \n " ) ;
//                   i_track->Print(30);
//                   Print(30);
...
*/
   if ( level > 9 ) {
      ...
      LOG(ERR,  " =======> Track      %d  <======== \n", id ,0,0,0,0) ;
      LOG(ERR,  " p,  pt, q         %7.2f  %7.2f  %2d \n", pmom, pt, q ,0,0) ;
   }
   if ( level > 19 ) {
     LOG(ERR, " r0,   z0,  nHits  %7.2f  %7.2f %d  \n  ", r0, z0, nHits,0,0 ) ;
     LOG(ERR, " phi0, psi, tanl   %7.2f  %7.2f %7.2f \n", phi0, psi, tanl,0,0 ) ;
   }

   if ( level > 29 ) {
     LOG(ERR," chi2 (s,z)        %6.2e  %6.2e \n", chi2[0], chi2[1],0,0,0 ) ;
   }

   if ( fmod((double)level,10.) > 0 ) {
     LOG(ERR," *** Clusters in this track *** \n",0,0,0,0,0 ) ;
...
void gl3Track::setDca(Ftf3DHit vertex)
{
    
    Ftf3DHit closest = closestApproach(vertex.x,vertex.y);
    dca = sqrt((closest.x - vertex.x)*(closest.x - vertex.x) +
	       (closest.y - vertex.y)*(closest.y - vertex.y) +
	       (closest.z - vertex.z)*(closest.z - vertex.z));
           

}

double gl3Track::nSigmaDedx(gl3Bischel* bischel, char* particle, double sigma1)
{
  double p = pt*sqrt(1.+pow(tanl, 2));
  double dedxBischel = bischel->getDedxMean(p, particle);
  if(dedxBischel == 0 || sigma1 ==0 || dedx == 0 || nDedx == 0) return -999.;
  else
    return log(dedx/dedxBischel)/sigma1*sqrt((double)nDedx);
}
