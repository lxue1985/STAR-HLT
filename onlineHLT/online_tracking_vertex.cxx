...
#endif
const float  online_tracking_vertex::C_PI=3.1416926;
//####################################################################
// Constructor
//####################################################################
online_tracking_vertex::online_tracking_vertex(int   _minHitsOnTrack,
int   _minCTBadc,
float _maxZdca,
float _zMargin,
float _phiMargin,
...
// Destructor
//####################################################################
online_tracking_vertex::~online_tracking_vertex()
...
//
//####################################################################
int online_tracking_vertex::setParameters(int   _minHitsOnTrack,
int   _minCTBadc,
float _maxZdca,
float _zMargin,
float _phiMargin,
...

{
    ...
    maxZdca        = _maxZdca;
    MatchCtbMaxZ   = CtbLen2/4. + _zMargin; 
    //(cm)  =slat+margin/cm
    MatchCtbMaxPhi = C_PI/30+_phiMargin/CtbRadius;
    //slat half width+margin/cm
    DelVertMax     = _delVertMax; 
    //(cm) rejection of outliers in ppLMV
    return 1;
...
//
//####################################################################
int online_tracking_vertex::setParameters(const char *filename)

{
    ...
    minHitsOnTrack = 10;
    minCTBadc      =  3;  
    // set to -1 if no CTB data avaliable, without CTB works only for low luminosity and bXing ID is not sure any more
    maxZdca    = 250.0;
    zMargin    =  10.0;
    phiMargin  =   3.0;
    delVertMax =   3.0;
    ...
    if(ret!=6) LOG(ERR,"online_tracking_vertex::setParameters() ERROR while reading %s file, ret=%d, default params used\n",filename,ret,0,0,0);
    if(minCTBadc<0) LOG(ERR, "online_tracking_vertex::setParameters() WARN : CTB matching is off\n",0,0,0,0,0);
...
//
//####################################################################
int online_tracking_vertex::init () 
...
//
//####################################################################
int online_tracking_vertex::registerHistos() 
...
//
//####################################################################
int online_tracking_vertex::makeVertex( online_tracking_collector *event ) 

{
    ...
    hjan[13]->Fill(1);
    hjan[1]->Fill(event->getNGlobalTracks(),1);
    if (event->getNGlobalTracks() > 5000)
    return 0;
    if( getCtbHits(event) <=0) return 0; 
    // no CTB slats found
    hjan[13]->Fill(2);
    ...
    hjan[2]->Fill(event->getNGlobalTracks(),1);
    //
    // loop over gtracks
    //
    for ( int trkcnt = 0 ; trkcnt <  event->getNGlobalTracks() ; 
    trkcnt++ ) 
    
    {

        hjan[14]->Fill(1);
        gl3Track* gTrack = event->getGlobalTrack(trkcnt);
        gTrack->flag=0;  
        // reset track flag to NULL
        ...
        { 
            ...
            if(fabs(hit.x)<0.1 && fabs(hit.y)<0.1) continue; 
            // extrapolation to CTB failed
            hjan[14]->Fill(4);
            if( matchTrack2Ctb(&hit) <0) continue; 
            // no match
            hjan[14]->Fill(5);
        ...
        { 
            ...
            // no matching to CTB, take all tracks
            hjan[14]->Fill(6);
    ...
    beamTrack.phi0=0;
    beamTrack.z0=0;
    beamTrack.tanl=888999.; 
    // almost parallel to Z
    beamTrack.psi=0;
    beamTrack.id=888999;
    beamTrack.lastHit=0;
    ...
    
    {
        ...
        if(trackL[j]->lastHit==NULL) continue; 
        // ignore 'beamTrack'
...
//
//####################################################################
int online_tracking_vertex::getCtbHits (online_tracking_collector *event) 
    ...
    
    {
        ...
        
        {
            ...
            { 
                ...
                //printf ( "online_tracking_vertex::getCtbHits ( ) Ctb index %d out of bounds, skip hit\n", indx ) ;
                continue ;
...
//
//####################################################################
int online_tracking_vertex::matchTrack2Ctb( Ftf3DHit * h) 
...
//
//####################################################################
int online_tracking_vertex::ppLMV4b (  Ftf3DHit *myV) 

{
    ...
    
    {
        ...
        { 
            ...
            double xhat=cos(trk->psi)/mag;    
            // wrong, temp not at DCA
            double yhat=sin(trk->psi)/mag;    
            // wrong, temp not at DCA
            ...
            */
            ...
            
            {
                ...
                sigma=0.0136 *Rxy/beta/trk->pt; 
                // average sigma is 2.2
            ...
            
            {
                ...
                if(trk->id==888999 && trk->tanl>88899.) sigma=0.5; 
                // 'beamTrack'
        ...
        C11=(A22*A33-A23*A32)/detA; 
        C12=(A13*A32-A12*A33)/detA; 
        C13=(A12*A23-A13*A22)/detA;
        C21=C12;                    
        C22=(A11*A33-A13*A31)/detA; 
        C23=(A13*A21-A11*A23)/detA;
        C31=C13;                    
        C32=C23;                    
        C33=(A11*A22-A12*A21)/detA;
    ...
    
    {
        ...
        << /*event->getNGlobalTracks() << */" L3-tracks" << endl
...
//
//####################################################################
void online_tracking_vertex::setCtbMap() 
